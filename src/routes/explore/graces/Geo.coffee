...
import { generateMemo } from '$lib/coffee/geoSixPhi.coffee'
...
export class Geo
  ...
  decode = 
    ...
    "p": -Phi
    "P": Phi
  # Helper: convert symbol to PhiBase
  symbolToPhiBase = (char) ->
    switch char
      when 'z' then new PhiBase(0, 0)
      when '0' then new PhiBase(0, 0)
      when 'O' then new PhiBase(0, 1)
      when 'o' then new PhiBase(0, -1)
      when 'P' then new PhiBase(1, 0)
      when 'p' then new PhiBase(-1, 0)
      when 'F' then new PhiBase(1, -1) # because 1/phi = phi - 1
      when 'f' then new PhiBase(-1, 1) # because -1/phi = -phi + 1
      else throw new Error("Invalid symbol: #{char}")

  # Create a SixPhiVector from '#symbol'
  symbolToSixPhi = (str) ->
    if str[0] != '#'
      throw new Error("String must start with '#'")

    chars = str.slice(1) # remove '#'

    if chars.length != 3
      throw new Error("Symbol must have exactly three characters after '#'")

    # Map into six components
    list = [
      symbolToPhiBase(chars[0]), # basis 0
      symbolToPhiBase(chars[0]), # basis 1
      symbolToPhiBase(chars[1]), # basis 2
      symbolToPhiBase(chars[1]), # basis 3
      symbolToPhiBase(chars[2]), # basis 4
      symbolToPhiBase(chars[2])  # basis 5
    ]

    new SixPhiVector(list)
