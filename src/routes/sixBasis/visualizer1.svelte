<script>
  ...
  function createPlaneAtEnd(basisVec, color) {
    const radius = 1.5;
    const segments = 32;
    const geometry = new THREE.CircleGeometry(radius, segments);
    const material = new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.25, side: THREE.DoubleSide });
    ...
    const position = norm.clone().multiplyScalar(Math.sqrt(basisVec.reduce((s, x) => s + x * x, 0)));
  ...
  function renderScene() {
    ...
    edges.forEach(({ a, b, basis }) => {
      ...
      scene.add(createPlaneAtEnd(sixBases[basis], color));
    });
