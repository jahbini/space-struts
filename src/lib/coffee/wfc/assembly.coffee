...
export class Assembly
  ...
  tryPlacement: (e, kind, edgeIdx, side, template, rejections = null) ->
    ...
    if existingC?
      ...
      # edge to any existing vertex. That distance signals a sub-scale gap
      # that only a smaller (1/φ²-scaled) tile could fill — and we're
      # single-scale, so leaving this placement out keeps the structure
      # clean rather than creating an unfillable hole.
      for v in @vertices
  ...
  # True if the candidate triangle overlaps any existing piece (shared
